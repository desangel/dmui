...
var fireNative, originaldmui, original$,
	baseURL = "",
	supportdmui = this.dmui,
...
// Setup global variables before loading dmui for testing .
originaldmui = this.dmui = undefined;
...
function match( message, selector, expectedIds, context ) {
	var f = dmui( selector, context ).get(),
		s = "",
		i = 0;
...
this.createWithFriesXML = function() {
	...
	return dmui.parseXML( string.replace( /\{\{\s*externalHost\s*\}\}/g, externalHost ) );
};
...
this.ajaxTest = function( title, expect, options ) {
	QUnit.test( title, expect, function( assert ) {
		...
		var completed = false,
			...
			requests = dmui.map( requestOptions, function( options ) {
				var request = ( options.create || dmui.ajax )( options ),
					callIfDefined = function( deferType, optionType ) {
						...
						return function( _, status ) {
							if ( !completed ) {
								...
								} else if ( dmui.isFunction( handler ) ) {
		...
		ajaxTest.abort = function( reason ) {
			if ( !completed ) {
				...
				dmui.each( requests, function( i, request ) {
					request.abort();
				} );
			}
		};
...
this.testIframe = function( title, fileName, func ) {
	QUnit.test( title, function( assert ) {
		...
		iframe = dmui( "<div/>" ).css( { position: "absolute", width: "500px", left: "-600px" } )
			.append( dmui( "<iframe/>" ).attr( "src", url( "./data/" + fileName ) ) )
	...
	require( [ "data/testrunner.js" ], function() {
		...
		( function loadDep() {
			...
			if ( dep ) {
				...
				if ( QUnit.isSwarm ) {
					require( [ "http://swarm.dmui.org/js/inject.js?" + ( new Date() ).getTime() ],
					function() {
						QUnit.start();
					} );
				} else {
